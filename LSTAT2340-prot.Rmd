...
We are to use the software infrastructure from the [R for Mass
Spectrometry](https://www.rformassspectrometry.org/) initiative to
load, handle and process mass spectrometry and proteomics data. This
document provides a short overview. For more details, consult the
[documentation book](https://rformassspectrometry.github.io/book/).

The *R for Mass Spectrometry* package are part of the Bioconductor
[@Huber:2015] project, and we'll use the `BiocManager` package,
(available from CRAN), to install them. If `BiocManager` isn't
available on your computer, install it with:
...
Now, install the `Spectra`, `mzR`, `PSMatch` and `QFeatures` packages
and their respective dependencies with

```{r setup2, eval = FALSE}
BiocManager::install(c("Spectra", "mzR", "PSMatch", "QFeatures"))
```

We will be using data accessible through the `MsDataHub` package.
...
```{r loadall, message = FALSE, warning = FALSE, echo = FALSE}
...
1. Let's start by downloading the data we need. The data are only
   downloaded one, and cached for later re-use. The command below will
   provide us with the path the an `mzML` file, an community-developed
   open format to mass spectrometry data.

```{r}
f <- MsDataHub::TMT_Erwinia_1uLSike_Top10HCD_isol2_45stepped_60min_01.20141210.mzML.gz()
```

2. We can now load the content of the file using the `Spectra`
   constructor from the `Spectra` package.
...
3. The object that is returned by `Spectra` is of class `Spectra`,
   that can store, access and manipulate raw MS data. Note that here
   we are focusing on MS-based proteomics data, but this also applied
   to MS-based metabolomics data.
...
table(msLevel(sp))
range(rtime(sp))
na.omit(precursorMz(sp))
na.omit(precursorIntensity(sp))
```

6. We can also extract subset of the `Spectra` object using `[` and plot them.
...
Let's load the `PSMatch` package that will allow us to handle
identificaiton data.
...
```{r calcfrag}
calculateFragments("IMIDLDGTENK")
```
...
s <- "IMIDLDGTENK"
plotSpectra(sp[24], label = addFragments)
```

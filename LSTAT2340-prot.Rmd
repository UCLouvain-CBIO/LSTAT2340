...
*mass spectrometry*. *A beginner's guide to mass spectrometry–based
proteomics* [@Sinha:2020] is an approachable introduction to sample
preparation, mass spectrometry and data analysis.


# Setup

We are going to use packages from the [R for Mass
Spectrometry](https://www.rformassspectrometry.org/) initiative for
this tutorial. For a more extensive introduction, readers are invited
to look at the [documetation
book](https://rformassspectrometry.github.io/book/), that provides
further details and state-of-the-art infrastructure for MS and
...
The *R for Mass Spectrometry* ecosystem is available as part of the
[Bioconductor project](https://bioconductor.org/) [@Huber:2015], and
we'll use the `BiocManager` package, available from CRAN. If
`BiocManager` isn't available on your computer, install it with:
...
Now, install the necessary packages, namely
- [MsDataHub](https://rformassspectrometry.github.io/MsDataHub/) to access data
- [Spectra](https://rformassspectrometry.github.io/Spectra/) for raw MS data
- [PSMatch](https://rformassspectrometry.github.io/PSMatch/) for identification data
- [QFeatures](https://rformassspectrometry.github.io/QFeatures/) for quantitative data

```{r setup2, eval = FALSE}
pkgs <- c("MsDataHub", "Spectra", "PSMatch", "QFeatures", "mzR")
...
We will use the `MsDataHub` package to access various types of
MS-based proteomics data: raw data, identification data, and
quantitative data.

```{r hub, message = FALSE}n
library("MsDataHub")
```

Here's a table with all data that can be accessed with the `MsDataHub`
package:

```{r hub2, message = FALSE}
DT::datatable(MsDataHub())
```

Each file can be downloaded (and cached) individually with a function
that has the same name as the file. For example, for the raw data
file:

```{r hub3}
(raw_file <- TMT_Erwinia_1uLSike_Top10HCD_isol2_45stepped_60min_01.20141210.mzML.gz())
```

# Reading and accessing MS data

Let's read a raw MS data file into R using the `Spectra()` function
...
2. Get the path to the file name:
...
The object that is returned is of class `Spectra`, that can store,
access and manipulate raw MS data. Note that here we are focusing on
MS-based proteomics data, but this also applied to MS-based
metabolomics data.
...
msLevel(x)[1000:1100]
rtime(x)[1000:1100]
precursorMz(x)[1000:1100]
precursorIntensity(x)[1000:1100]
```

6. We can also extract `Spectra` substets using `[` and plot them.

```{r}
x[1000]
```

```{r, fig.cap = "Visualisation of the 1000th MS spectrum in our test data set."}
plotSpectra(x[1000])
```

## Exercise

Calculate the number of MS1 and MS2 scans and the retention time range
over which these spectra have been acquired.

<details>
```{r}
table(msLevel(x))
formatRt(range(rtime(x)))
MsCoreUtils::formatRt(range(rtime(x)))
```
</details>

This object also contains additional metadata for each spectrum, that
can be accessed, as a `DataFrame`, with `spectraData()`.
...
Let's load a identification result file:

```{r idf}
(id_file <- MsDataHub::TMT_Erwinia_1uLSike_Top10HCD_isol2_45stepped_60min_01.20141210.mzid())
id <- PSM(id_file)
id
```


Here, we are going to do a quick filter (more details in the [package
vignette](https://rformassspectrometry.github.io/PSMatch/articles/PSM.html)),
to keep only the most reliable identification results.

```{r filter}
id_filtered <- PSMatch::filterPSMs(id)
```

Let's add the matches/identifications to the raw data...

```{r join}
spectraVariables(x) ## before
x <- joinSpectraData(x, id_filtered,
                     by.x = "spectrumId",
                     by.y = "spectrumID")
spectraVariables(x) ## after
```


... and plot on MS2 with it's identification results:

```{r plotwithid}
plotSpectra(sp[983], labels = addFragments,
...
The sequence matched to spectrum 983 can be accessed from the raw data:

```{r}
x$sequence[983]
```

Find a second scan that was matched that same peptide sequence, and
compare both with `Spectra::plotSpectraMirror()`.


<details>
```{r}
(i <- which(sp$sequence == "THSQEEMQHMQR"))
Spectra::plotSpectraMirror(x[i[1]], x[i[2]])
```
</details>
